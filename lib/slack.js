...
// 크몽 알림은 #크몽알림 채널 전용 웹훅(KMONG_SLACK_WEBHOOK_URL)을 우선 사용하고,
// 미설정 시 위시켓 봇과 공유하는 SLACK_WEBHOOK_URL(#위시켓-알림)로 폴백한다.
// 웹훅 URL은 GitHub secret scanning 대상이라 레포에 하드코딩하지 않는다 — 배포 환경(Routine 등)에
// 환경변수로 설정할 것.
const SLACK_WEBHOOK_URL = process.env.KMONG_SLACK_WEBHOOK_URL || process.env.SLACK_WEBHOOK_URL || '';
